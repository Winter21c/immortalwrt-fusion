...
| 验证项 | 状态 |
|---|---|
| 四种勾选组合各自拼出的 `.config` | 逐项断言（该有的在、不要的不在） |
| 镜像配置（只为 4 个镜像） | 断言 TARGZ / INITRAMFS / CPIOGZ / ISO 均为关 |
| 编译后固件里真实的包列表 | 对照 `*.manifest` 核对 |
| 镜像 sha256 校验和 | 发布前核对 |
| **完整编译过一次** | 见 `docs/BUILD-NOTES.md` —— 那里记录了实测结果与耗时 |

**没法在这里验证的**（刷到真机后请自行确认）：

- fwx 的应用识别需要真实流量才有意义，空跑看不出来
- iStore 能否拉到应用列表取决于外网连通性，与固件无关
- OpenClash 内核需要联网下载，且分流效果取决于你的订阅规则
